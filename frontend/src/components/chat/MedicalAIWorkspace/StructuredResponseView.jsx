...
export default function StructuredResponseView({ message, onSelectQuestion, onAction }) {
  ...
  // Safe extraction matching Part 8 Executive Workstation Schema
  ...
  const disclaimer = content.disclaimer || 'Note: HealthLens AI provides automated analytical insights strictly for educational reference.';
  ...
  const handleExport = (format) => {
    const text = `HEALTHLENS EXECUTIVE CLINICAL REPORT\nFormat: ${format}\n\nSummary:\n${summary}\n\nKey Meaning:\n${meaning.join('\n')}`;
    const blob = new Blob([text], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `HealthLens_Executive_Report_${Date.now()}.${format.toLowerCase()}`;
    a.click();
  };
