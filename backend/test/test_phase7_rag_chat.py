...
def test_5layer_guardrails():
    ...
    assert "HealthLens AI" in med_text or "healthcare provider" in med_text
...
def test_end_to_end_medical_chat_engine(db_session):
    user = db_session.query(User).filter(User.email == "phase7_test@healthlens.ai").first()
    if not user:
        user = User(name="Phase7 User", email="phase7_test@healthlens.ai", password_hash="hash")
        db_session.add(user)
        db_session.commit()
